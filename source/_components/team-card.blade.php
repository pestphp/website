<div class="flex flex-col items-center justify-start font-sans">
	...
	<div class="flex flex-col items-center justify-center text-center">
		...
		<div class="mt-4 flex space-x-3 items-center flex-row">
			@if($contributor->has('github'))
			<a title="GitHub" aria-label="GitHub" href="https://github.com/{{ $contributor->github }}">
				<x-i-github class="w-5 text-black" />
			</a>
			@endif

			@if($contributor->has('twitter'))
			<a title="Twitter" aria-label="Twitter" href="https://twitter.com/{{ $contributor->twitter }}">
				<x-i-twitter class="w-5 mt-px" />
			</a>
			@endif

			@if($contributor->has('links'))
			@foreach($contributor->links as $link)
			<a aria-label="{{ $link->website }}" href="{{ $link->url }}">
